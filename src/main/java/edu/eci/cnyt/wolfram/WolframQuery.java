...
public class WolframQuery {
    ...
    // PUT YOUR APPID HERE:
    private static final String appid = "AQ9YJ3-6TU7V3VUL5";
    private List<String> resultado;
    public WolframQuery(){
        
    }
    public List<String> vectoresPropios(String entrada) {
        resultado = new ArrayList<>();
        // Use "pi" as the default query, or caller can supply it as the lone command-line argument.
        String input = "eigenvectors " + entrada;
                        ...
                        for (WASubpod subpod : pod.getSubpods()) {
                            for (Object element : subpod.getContents()) {
                                if (element instanceof WAPlainText) {
                                    resultado.add(((WAPlainText) element).getText().substring(6));
                                }
                            }
                        }
                    }
